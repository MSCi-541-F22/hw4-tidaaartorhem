# Aadit Mehrotra 20756049
from collections import OrderedDict
import getDocument
import math

class Measures:
    def __init__(self, qrel, global_id, results, n):
        self.n = n
    ...
    def __calculate_average_precision(self):
        ...
        for query_id in self.qrel.get_query_ids():
            ...
            if result:
                ...
                avg_precision[query_id] = float(0)
    ...
    def __calculate_precision_at_10(self):
        ...
        for query_id in self.qrel.get_query_ids():
            ...
            if result:
                ...
                precision_at_10[query_id] = float(0)
    ...
    def __calculate_ndcg_10_1000(self):
        ...
        for query_id in self.qrel.get_query_ids():
            ...
            if result:
                ...
                ndcg_10[query_id] = float(0)
                ndcg_1000[query_id] = float(0)
    ...
    def __calculate_time_based_gain(self):
        ...
        for query_id in self.qrel.get_query_ids():
            ...
            if result:
                ...
                for i, r in enumerate(result, start=1):
                    if(self.n == 'hw4-bm25-stem-asmehrot.txt'):
                        document = getDocument.retrieve_by_docno('./documentsStem', r.doc_id)
                    else:
                    ...
                    if self.qrel.get_relevance(query_id, r.doc_id) > 0:
                        p = 0.64
                        g_k = p * 0.77
                        t_k += 4.4 + (0.018 * document.length + 7.8) * p
                        d_t_k = math.exp(-t_k*(math.log(2)/224))
                        gain = g_k * d_t_k
                        tbg += gain
                    else:
                        p = 0.39
                ...
                time_based_gain[query_id] = float(0)
